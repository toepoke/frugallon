...
import { IONIC_DIRECTIVES, SegmentButton } from "ionic-angular";
...
@Component({
	...
	template: `
		<ion-segment [hidden]="!showControl()" [(ngModel)]="selectedValue" (click)="onSegmentClick($event)" (ionChange)="onSegmentChanged($event)">
			<button clear [hidden]="!showNavigation()" [disabled]="!enablePrev()" (click)="goPrev()">
			...
			<button clear [hidden]="!showNavigation()" [disabled]="!enableNext()" (click)="goNext()">
